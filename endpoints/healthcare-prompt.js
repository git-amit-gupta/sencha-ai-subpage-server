...
// The PROMPT
    function createShortPrompt(userQuery) {
        return `
            ...
            Fields: employee_id (str), name (str), department (list:Cardiology/Dematology/Emergency/ICU/Lab/Neurology/Oncology/Orthopedics/Pediatric/Radiology/Surgery), role (list: Doctor/Nurse/Technician), shift (list: Morning/Evening/Night), date (date), duration_hours (num), status (list: Assigned/On leave/Available)
            ...
            Examples:
                prompt: show personnel available between 5 and 10 hours
                output: { "property": "duration", "value": { "gt": 5, "lt": 10 }, "type": "number" }
                prompt: show users with name keith
                output: { "property": "name", "value": "keith", "operator": "like", "type": "string" }
                prompt: show available users for march 2021
                output: { "property": "date", "value": { "gt": "2021-03-01", "lt": "2021-03-31" }, "type": "date" }
                prompt: show avaliable for march 21st, 2021
                output: { "property": "date", "value": { "eq": "2021-03-21" }, "type": "date" }
                prompt: show available for on 2021
                output: { "property": "date", "value": { "gt": "2021-01-01", "lt": "2021-12-31" }, "type": "date" }
